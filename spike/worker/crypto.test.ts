...
describe('envelope encryption', () => {
  it('round-trips and binds AAD and KEK', async () => {
    const kek = await importKek(KEK); const dek = await newDek()
    ...
    await expect(unwrapDek(await importKek('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA='), w.wrapped, w.iv, 'you@example.com')).rejects.toThrow()
  })
})
