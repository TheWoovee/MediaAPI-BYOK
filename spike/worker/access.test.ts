...
const sign = (pk: CryptoKey, claims: Record<string, unknown>, opts: { iss?: string; aud?: string; exp?: string } = {}) =>
  new SignJWT(claims).setProtectedHeader({ alg: 'RS256', kid: 'k1' }).setIssuer(opts.iss ?? env.ACCESS_TEAM_DOMAIN)
    .setAudience(opts.aud ?? env.ACCESS_AUD).setIssuedAt().setExpirationTime(opts.exp ?? '10m').sign(pk)
const req = (token?: string, host = 'www.thewoovee.com') =>
  new Request(`https://${host}/studio/api/me`, { headers: token ? { 'Cf-Access-Jwt-Assertion': token } : {} })
...
describe('Access JWT verification', () => {
  ...
  it('rejects wrong audience, wrong issuer, expired, and foreign key', async () => {
    ...
    expect(await identify(req(await sign(privateKey, { email: 'a@b.c' }, { iss: 'https://evil.cloudflareaccess.com' })), env, getKey)).toBeNull()
  ...
  it('refuses requests without a token and never trusts DEV_TRUST_EMAIL off localhost', async () => {
    ...
    expect(await identify(req(undefined, 'localhost'), { ...env, DEV_TRUST_EMAIL: 'dev@x' }, getKey)).toEqual({ email: 'dev@x' })
  })
})
